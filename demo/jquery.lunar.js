...
jQuery.extend({
    ...
    getLunarObj: function(opt){
        ...
        if(options.week){
            viewdate.week = get_week(options.dateObj);
        }

        //农历信息 year .month .day .isLeap .yearCyl .dayCyl .monCyl
        var lunar_obj = new lunar(options.dateObj);


        if(options.lunarMonth){
        ...
        //农历年月日
        if(options.lunarDate){
            viewdate.lunarDate = lunar_obj.year + "-" + lunar_obj.month + "-" + lunar_day;
        }
        ...
        //生肖
        if(options.zodiac){
            viewdate.zodiac = get_zodiac(lunar_obj.year);
        }
